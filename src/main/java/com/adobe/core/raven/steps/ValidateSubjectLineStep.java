...
public class ValidateSubjectLineStep implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        String subjectLineFromCgen = cgenService.fetchSubjectline(activityID,cgenContent);
        ...
        if( !ObjectUtils.isEmpty(masterJob.getLifecycleMarket())
                && masterJob.getLifecycleMarket().equals("Email - Tech Mktr")
                &&
                !ObjectUtils.isEmpty(masterJob.getLifecycleMarket())&&subjectLine.contains("-")){
                int firstHyphenIndex = subjectLine.indexOf('-');
            subjectLine = subjectLine.substring(firstHyphenIndex + 1).trim();
        }
        ...
        if(subjectLineFromCgen != null
                && subjectLine != null
                && validateService.compare(subjectLineFromCgen
