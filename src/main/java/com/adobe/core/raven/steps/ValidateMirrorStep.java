...
public class ValidateMirrorStep implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
        ...
        String mirrorTextFromRepo=null;

        if(!ObjectUtils.isEmpty(masterJob.getLifecycleMarket())
                && masterJob.getLifecycleMarket().equalsIgnoreCase("Email - Tech Mktr")){
            mirrorTextFromRepo  = dataService.fetchMirrorTechMarketing(locale);
        }else {
            mirrorTextFromRepo = dataService.fetchMirror(locale);
        }
        ...
        if(!ObjectUtils.isEmpty(mirrorTextFromRepo) && validateService.compare(mirrorTextFromRepo
